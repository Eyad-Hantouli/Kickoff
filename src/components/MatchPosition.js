import React, { Fragment, useState } from "react";
import { useNavigate, useParams } from "react-router-dom";
import "../styles/match_position.css";
import { Relations } from "../RelationsEnum";
import axios from "axios";

const MatchPosition = ({ teamNumber, positionNumber, position, match, source_user, target_user, register }) => {

    const navigate = useNavigate();

    const defaultProfileImage = "data:image/png;base64,";
    console.log(target_user);
    const handleRegister = async () => {
        if (!register) return;

        try {
            const response = await axios.delete(`http://localhost:8080/system/delete-registration-in-match/schedule/${match.id}/user/${source_user.username}`, {
            });

          } catch (error) {
            console.error("Error in sending contact us message");
          }

        try {
            const response = await axios.post('http://localhost:8080/system/register-in-match', {
                scheduleId: parseInt(match.id),
                username: source_user.username,
                position: position,
                team_number: teamNumber,
                position_number: positionNumber
            });

            navigate(-1);

          } catch (error) {
            console.error("Error in sending contact us message");
          }
    }

    if (!target_user) return <span onClick={handleRegister} className={register ? "clickable" : ""}></span>;


    return (
        <div className={`MatchPosition ${source_user.username === target_user.username ? "you" : ""}`} style={{backgroundImage: `url(${target_user.profileImage.length > 0 ? "data:image/jpeg;base64,"+target_user.profileImage : defaultProfileImage})`}} onClick={() => {navigate(`/profile/${target_user.username}`)}}>
            {
                target_user.relation_type === Relations.FRIEND
                ? <div className="tagger yellow"><i className="fa-solid fa-star"></i></div>
                : target_user.relation_type === Relations.BLOCK
                ? <div className="tagger red"><i className="fa-solid fa-ban"></i></div>
                : <></>
            }
        </div>
    );
};

export default MatchPosition;
